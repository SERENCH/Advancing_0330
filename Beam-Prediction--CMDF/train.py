...
parser.add_argument('--persistent_workers', type=int, default=1, help='Keep DataLoader workers alive between epochs')
parser.add_argument('--prefetch_factor', type=int, default=2, help='Prefetch batches per worker when num_workers > 0')
parser.add_argument('--amp', type=int, default=1, help='Enable CUDA automatic mixed precision')
parser.add_argument('--data_parallel', type=int, default=1, help='Enable DataParallel when multiple CUDA devices are available')
...
args.use_amp = bool(args.amp) and use_cuda
args.use_data_parallel = bool(args.data_parallel) and use_cuda and torch.cuda.device_count() > 1
args.pin_memory = use_cuda
...
def build_dataloader(dataset, batch_size, shuffle=False, sampler=None, worker_init_fn=None,
                     generator=None):
    loader_kwargs = {
        'dataset': dataset,
...
def load_model_weights(model, checkpoint_path):
    ...
    checkpoint_is_dp = any(key.startswith('module.') for key in state_dict.keys())

    if checkpoint_is_dp and not model_is_dp:
        state_dict = {key.replace('module.', '', 1): value for key, value in state_dict.items()}
    elif model_is_dp and not checkpoint_is_dp:
    ...
    def train(self):
        ...
        for data, batch_idx in pbar:
            ...
            for i in range(config.seq_len):
                fronts.append(data['fronts'][i].to(args.device, dtype=torch.float32, non_blocking=args.pin_memory))
                radars.append(data['radars'][i].to(args.device, dtype=torch.float32, non_blocking=args.pin_memory))
            gt_beamidx = data['beamidx'][0].to(args.device, dtype=torch.long)
            gt_beams = data['beam'][0].to(args.device, dtype=torch.float32, non_blocking=args.pin_memory)
            with torch.amp.autocast(device_type=AMP_DEVICE_TYPE, enabled=args.use_amp):
                fused_pred, image_pred, radar_pred, image_conf, radar_conf = model(fronts, radars)
                fused_pred_fp32 = fused_pred.float()
                image_pred_fp32 = image_pred.float()
                radar_pred_fp32 = radar_pred.float()
                if args.temp_coef:  # temperature coefficiece
                    image_clf_loss = self.criterion(image_pred_fp32, gt_beams)
                    radar_clf_loss = self.criterion(radar_pred_fp32, gt_beams)
                    fused_clf_loss = self.criterion(fused_pred_fp32, gt_beams)
                    loss = fused_clf_loss + image_clf_loss + radar_clf_loss
                else:
                    image_clf_loss = self.criterion(image_pred_fp32, gt_beamidx)
                    radar_clf_loss = self.criterion(radar_pred_fp32, gt_beamidx)
                    fused_clf_loss = self.criterion(fused_pred_fp32, gt_beamidx)
                    loss = fused_clf_loss + image_clf_loss + radar_clf_loss
            running_acc += (torch.argmax(fused_pred, dim=1) == gt_beamidx).sum().item()

            image_loss = self.criterion(image_pred_fp32, gt_beamidx).detach()
            radar_loss = self.criterion(radar_pred_fp32, gt_beamidx).detach()
            ...
            pred_beam_all.append(torch.argsort(fused_pred_fp32, dim=1, descending=True).cpu().numpy())
        ...
        with torch.no_grad():
            ...
            for batch_num, data in enumerate(tqdm(dataloader_test), 0):
                ...
                with torch.amp.autocast(device_type=AMP_DEVICE_TYPE, enabled=args.use_amp):
                    ...
                    pred_beams_fp32 = pred_beams.float()
                    image_pred_fp32 = image_pred.float()
                    radar_pred_fp32 = radar_pred.float()
                    if args.temp_coef:  # temperature coefficiece
                        image_clf_loss = self.criterion(image_pred_fp32, gt_beams)
                        radar_clf_loss = self.criterion(radar_pred_fp32, gt_beams)
                        fused_clf_loss = self.criterion(pred_beams_fp32, gt_beams)
                        loss = image_clf_loss + radar_clf_loss + fused_clf_loss
                    else:
                        image_clf_loss = self.criterion(image_pred_fp32, gt_beamidx)
                        radar_clf_loss = self.criterion(radar_pred_fp32, gt_beamidx)
                        fused_clf_loss = self.criterion(pred_beams_fp32, gt_beamidx)
                        loss = image_clf_loss + radar_clf_loss + fused_clf_loss
                pred_beam_all.append(torch.argsort(pred_beams_fp32, dim=1, descending=True).cpu().numpy())
...
if args.finetune and not args.Test:
    ...
    if __name__ == '__main__':
        dataloader_val = build_dataloader(val_set, batch_size=args.batch_size, shuffle=False)
...
if args.Test:
    if __name__ == '__main__':
        dataloader_test = build_dataloader(test_set, batch_size=args.batch_size, shuffle=False)
else:
    if __name__ == '__main__':
...
if args.use_data_parallel:
    ...
    print('======DataParallel disabled; using single-device execution.')
optimizer = optim.AdamW(model.parameters(), lr=args.lr, weight_decay=0.01, amsgrad=False)
scaler = torch.amp.GradScaler(AMP_DEVICE_TYPE, enabled=args.use_amp)
print(
    f'======Runtime config: device={args.device} amp={args.use_amp} '
    f'num_workers={args.num_workers} persistent_workers={bool(args.persistent_workers) and args.num_workers > 0} '
...
if not os.path.isdir(args.logdir):
    ...
    if args.finetune:  # finetune the pretrained model
        ...
        if os.path.exists(os.path.join(args.logdir, 'all_finetune_on_' + kw + 'model.pth')):
            ...
            load_model_weights(model, os.path.join(args.logdir, 'all_finetune_on_' + kw + 'model.pth'))
            optimizer.load_state_dict(torch.load(os.path.join(args.logdir, 'all_finetune_on_' + kw + 'optim.pth')))
        else:
            print('======loading ' + kw + ' model')
            load_model_weights(model, os.path.join(args.logdir, kw + 'model.pth'))
    else:
        print('======loading best_model')
        load_model_weights(model, os.path.join(args.logdir, 'best_model.pth'))
